package com.ambabovpn.pro.adapter;
...
import com.ambabovpn.tunnel.logger.LogItem;
		...
		View.OnTouchListener
{
	...
	{ 
		...
		if (new Setting(mContext).getDebugMode()) {
	...
	public void onBindViewHolder(@NonNull final logViewHolder viewHolder, final int position) {
		...
		viewHolder.textLog.setOnClickListener(v -> {
			if (itemClickListener != null)
				itemClickListener.onItemClick(v, viewHolder.getAbsoluteAdapterPosition(), text);
		});

		viewHolder.textLog.setOnLongClickListener(v -> {
			if (itemClickListener != null)
				itemClickListener.onItemLongClick(v, viewHolder.getAbsoluteAdapterPosition(), text);
			return true;
		});
	}
