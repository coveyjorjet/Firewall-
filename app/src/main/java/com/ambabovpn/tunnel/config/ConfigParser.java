package com.ambabovpn.tunnel.config;
...
import com.ambabovpn.tunnel.util.Cripto;
import com.ambabovpn.tunnel.util.FileUtils;
import com.ambabovpn.tunnel.util.securepreferences.crypto.Cryptor;
import com.ambabovpn.tunnel.util.securepreferences.model.SecurityConfig;
...
public class ConfigParser
{
	...
	public static boolean convertInputAndSave(InputStream input, Context mContext)
		...
		SharedPreferences.Editor prefsEdit = setting.getPrefsPrivate()
			.edit();
		...
		try {
			...
			try {
				...
				prefsEdit.putBoolean(Setting.PROXY_USAR_DEFAULT_PAYLOAD, !mConfigFile.getProperty(Setting.PROXY_USAR_DEFAULT_PAYLOAD).equals("1") ? false : true);
				...
				if (mIsProteger) {
					...
					if (pedirLogin != null)
						prefsEdit.putBoolean(Setting.CONFIG_INPUT_PASSWORD_KEY, pedirLogin.equals("1") ? true : false);
					else
				...
				setting.setVpnDnsForward(isDnsForward);
				setting.setVpnDnsResolver(dnsResolver);
				...
				setting.setVpnUdpForward(isUdpForward);
				setting.setVpnUdpResolver(udpResolver);
				
			} catch(Exception e) {
				if (setting.getDebugMode()) {
					SkStatus.logException("Error Setting", e);
				}
	...
	public static void convertDataToFile(OutputStream fileOut, Context mContext,
		...
		Setting setting = new Setting(mContext);
		SharedPreferences prefs = setting.getPrefsPrivate();
		...
		try {
			...
			mConfigFile.setProperty(Setting.DNSFORWARD_KEY, setting.getVpnDnsForward() ? "1" : "0");
			mConfigFile.setProperty(Setting.DNSRESOLVER_KEY, setting.getVpnDnsResolver());
			
			mConfigFile.setProperty(Setting.UDPFORWARD_KEY, setting.getVpnUdpForward() ? "1" : "0");
			mConfigFile.setProperty(Setting.UDPRESOLVER_KEY, setting.getVpnUdpResolver());
	...
	*/
	...
	public static boolean isDeviceRooted(Context context) {
		...
		boolean simpleTests = rootBeer.detectRootManagementApps() || rootBeer.detectPotentiallyDangerousApps() || rootBeer.checkForBinary("su")
			...
			|| rootBeer.detectTestKeys() || rootBeer.checkSuExists() || rootBeer.checkForRootNative() || rootBeer.checkForMagiskBinary() || rootBeer.isRooted() || rootBeer.checkForMagiskNative();
